...
static esp_err_t start_saved_wifi_mode(const wifi_cfg_t *cfg) {
    esp_err_t err = server_notify_check_configured();
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "SMS Gateway notify config invalid: %s",
                 esp_err_to_name(err));
    ...
    err = server_notify_start_heartbeat();
    if (err != ESP_OK) {
...
static esp_err_t start_portal_mode(void) {
    ESP_ERROR_CHECK(wifi_manager_init());
    ESP_ERROR_CHECK(web_server_start());
...
void app_main(void) {
    ...
    switch (startup_mode) {
        case STARTUP_MODE_SAVED_WIFI:
            err = start_saved_wifi_mode(&saved_wifi_cfg);
            break;
        case STARTUP_MODE_PORTAL:
            err = start_portal_mode();
            break;
        default:
            return;
    }
